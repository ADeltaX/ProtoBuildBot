...
namespace Schedule
{
	...
	{
		public ScheduledTime(EventTimeBase Base, TimeSpan Offset, bool isPT = false)
		{
			//TODO: remember daylight saving time

			_Base = Base;
			_Offset = Offset;
		}

		public static TimeSpan GetUtcTimeSpanFromLocal(TimeSpan timeSpan)
		{
			var timeOffset = TimeZoneInfo.Local.GetUtcOffset(DateTime.UtcNow);
			return DateTime.MinValue.Add(TimeSpan.FromDays(2)).Subtract(timeOffset).Add(timeSpan).TimeOfDay;
		}

		public static TimeSpan GetLocalTimeSpanFromPT(TimeSpan timeSpan) 
			=> GetLocalTimeSpanFromTimeZone(timeSpan, TZConvert.GetTimeZoneInfo("Pacific Standard Time"));

		public static TimeSpan GetLocalTimeSpanFromUtc(TimeSpan timeSpan)
		{
			var timeOffset = TimeZoneInfo.Local.GetUtcOffset(DateTime.UtcNow);
			return DateTime.MinValue.Add(TimeSpan.FromDays(2)).Add(timeOffset).Add(timeSpan).TimeOfDay;
		}

		private static TimeSpan GetLocalTimeSpanFromTimeZone(TimeSpan timeSpan, TimeZoneInfo timeZoneInfo)
		{
			return new DateTimeOffset(DateTime.MinValue.AddDays(2).Add(timeSpan),
				timeZoneInfo.GetUtcOffset(DateTime.UtcNow)).LocalDateTime.TimeOfDay;
		}
