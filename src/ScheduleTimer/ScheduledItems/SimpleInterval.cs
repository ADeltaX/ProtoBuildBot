...
namespace Schedule
{
	...
	public class SimpleInterval : IScheduledItem
	{
		...
		{
			TimeSpan elapsedSinceStart = time - _StartTime;
			...
			long ticksRemaining = _Interval.Ticks - (elapsedSinceStart.Ticks % _Interval.Ticks);

			return time.AddTicks(ticksRemaining);
		}

		private bool ExactMatch(DateTime time)
		{
			long ticksElapsed = time.Ticks - _StartTime.Ticks;

			if (ticksElapsed < 0)
				return false;

			return (ticksElapsed % _StartTime.Ticks) == 0;
		}
